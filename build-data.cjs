...
// V90.2: READ FROM public/data FOR VERCEL BUILD.
// Expo still uses assets/data via require() in app/index.tsx
...
const OUT_FILE = path.join('public', 'data', 'knowledge-lite.json');

function buildLite() {
  if (!fs.existsSync(ROOT_DIR)) {
    console.error('ERROR: public/data folder not found at', ROOT_DIR);
    console.log('Hint: Run: robocopy assets\\data public\\data /E /IS /IT');
    process.exit(1);
  }

  if (!fs.existsSync(INDEX_PATH)) {
    console.error('ERROR: schools/index.json not found at', INDEX_PATH);
    console.log('Hint: Make sure public/data/schools/index.json exists');
    process.exit(1);
  }
  ...
  // Load all school files from public/data/schools/
  let allSchools = [];
  for (const prov of Object.keys(index)) {
    const file = index[prov].file; // e.g. /data/schools/schools_eastern_cape.json
    // V90.2: resolve from public instead of assets
    const filePath = path.join(__dirname, 'public', file.replace('/data/', 'data/'));
