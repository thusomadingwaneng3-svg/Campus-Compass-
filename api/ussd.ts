...
function loadData() {
  try {
    return {
      ...
      bursaries: JSON.parse(fs.readFileSync(path.join(dataDir, 'africa_busaries.json'), 'utf8')),
...
export async function POST(req: Request) {
  ...
  const data = loadData();
  if (!data) {
    return new Response('END Server error. Check JSON files.', {
...
export async function GET() {
  return new Response('Campus Compass USSD API is live');
}